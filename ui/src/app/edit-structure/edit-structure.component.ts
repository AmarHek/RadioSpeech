...
export class EditStructureComponent implements OnInit, OnDestroy {
  ...
  onSubmitKat() {
    ...
    let catName = (<HTMLInputElement>document.getElementById("disName"))
      .value;
    let sels: Array<N.Selectable> = [];
    // this loop gets all the data from the input Form by the assigned IDs. Empty Variables, Attributes and a Category are created and filled with the input Form informations.
    ...
    for (
      ...
      let vars: Array<N.Variable> = [];
      let attText = (<HTMLInputElement>(
        document.getElementById("KAttText" + i)
      )).value;
      let attName = (<HTMLInputElement>(
        document.getElementById("KAttName" + i)
      )).value;
      let attJud = (<HTMLInputElement>(
        document.getElementById("KJudText" + i)
      )).value;
      let attNormal = (<HTMLInputElement>(
      ...
      for (
        ...
        if (
          ...
          let myTextBefore = (<HTMLInputElement>(
            document.getElementById("KtextBefore" + i + k)
          )).value;
          let myTextAfter = (<HTMLInputElement>(
            document.getElementById("KtextAfter" + i + k)
          )).value;
          let varUnit = (<HTMLInputElement>(
            document.getElementById("KvarUnit" + i + k)
          )).value;
          let myTextVar = <N.VariableText>{
            kind: "text",
          ...
          let varOptions: Array<String> = (<HTMLInputElement>(
            document.getElementById("KvarOptions" + i + k)
          )).value
            .split("/")
            .map((opt) => opt.trim());
          let myOCVar = <N.VariableOC>{ kind: "oc", values: varOptions };
          vars.push(myOCVar);
        }
      }
      let myAtt = <N.Selectable>{
        kind: "box",
    ...
    let myCat = <N.Category>{
      kind: "category",
      name: catName,
      selectables: sels,
    };
    let positions = this.addPosition.split("").map(Number);
    let myVar: N.TopLevel = this.myEdit.dict[positions[0]];
  ...
  deleteDisease(index: Array<number>) {
    ...
    let myVar: N.TopLevel = this.myEdit.dict[index[0]];
    switch (index.length) {
      case 1:
  ...
  renameTop(ind: Array<number>, c, option: string) {
    let myVar: N.TopLevel = this.myEdit.dict[ind[0]];
    switch (ind.length) {
      case 1:
      ...
      case 3:
        if (myVar.kind == "disease") {
          ...
          let myVar2 = myVar.selectables[ind[1]].variables[ind[2]];
          if (myVar2.kind == "oc") {
  ...
  // this function manages the initiation process when anything is added to an existing dictionary. One case for variables, attributes and categories
  addPosition = "";
  addVar = false;
  addAtt = false;
  addKat = false;
